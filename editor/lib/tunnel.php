<?php
	...
	class Tunnel {
		...
		function startTunnel($forwardID) {				
			...
			$command = "{$location_autossh} {$forward['SSHUser']}@{$forward['SSHHostName']} -p {$forward['SSHPort']} " . ($proxyEnabled ? $proxycommand : "") . " -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -g -4 -{$type}{$forward['localPort']}:{$forward['remoteTargetHost']}:{$forward['remoteTargetPort']} -N {$compression} {$keepalive} {$serveralive} -M0 &";					
