...
class lazy_mofo{
	public $button = "<input type='submit' name='__update_grid' value='Save changes' class='lm_button lm_save_changes_button'>";
    ...
    public $form_display_identity = false;   // display identity value when editing a record
    ...
    public $form_text_title_add    = 'Add';   // titles in the <th> of top of the edit form 
    public $form_text_title_edit   = 'Edit';
    public $form_text_record_saved = 'Saved'; // customize success messages
    public $form_text_record_added = 'Added';

    public $grid_add_link    = "<a href='[script_name]action=edit&amp;[qs]' class='lm_grid_add_link'>Add new</a>";  // link at displayed at the top to add a new record. [script_name] placeholder  will be populated by grid()
    ...
    function __construct($dbh, $optLinks = ""){
    ...
    function update(){
        ...
        if($this->after_update_user_function != '')
            call_user_func($this->after_update_user_function, $id);
    ...
    function delete(){
        ...
        if($this->on_delete_user_function != '')
            $error = call_user_func($this->on_delete_user_function, $identity_id);
    ...
    function form($error = ''){
        ...
        $html  = "<div id='lm'>\n";
        global $page;
        $html .= "<form action='$uri_path$qs&page=" . $page . "' method='post' enctype='multipart/form-data'>\n";
        $html .= "<input type='hidden' name='_csrf' value='$_SESSION[_csrf]'>\n";
        $html .= "<input type='hidden' name='_posted' value='1'>\n";
        $html .= "<input type='hidden' name='page2' value='projects'>\n";
        ...
        $html .= "<table cellpadding='2' cellspacing='1' border='0' class='table table-striped'>\n";
    ...
    function grid($error = ''){
		...
		if(!$this->paginate) $_pagination_off=1;
        ...
        $grid_add_link = $this->optLinks . $this->grid_add_link;
		...
		$pagination_button_bar = "<table cellpadding='2' cellspacing='1' border='0' width='100%' class='lm_pagination'><tr><td style='text-align: left'>$pagination</td><td style='text-align: right'>{$this->button}</td></tr></table>\n";
		...
		$add_record_search_bar = "<table cellpadding='2' cellspacing='1' border='0' width='100%' class='lm_add_search'><tr><td style='text-align: left'>$grid_add_link &nbsp;</td><td style='text-align: right'>$grid_export_link $search_box</td></tr></table>\n";
        ...
        $html .= "<table cellpadding='2' cellspacing='1' border='0' width='100%' class='table table-striped'>\n";
        $html .= $head;
	...
	//Changed LM to not show any pagination when forcibly turned off
		if($_pagination_off) return;
